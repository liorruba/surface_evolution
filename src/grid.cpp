...
void Grid::formCrater(Crater &crater){
  ...
  cratersVector.push_back(&crater);

  // Add crater location and index to r-tree:
  rtree.insert({cratersVector.size() - 1, {crater.xLocation, crater.yLocation}});
...
void Grid::updateExistingCratersDepth(Crater &crater) {
  ...
  // when the distance from the new impact point exceeds the crater radius.
    for ( rtree_t::const_query_iterator
            it = rtree.qbegin(bgi::nearest(query_pt, cratersVector.size()));
            it != rtree.qend() ;
            ++it ) {
        ...
        // the crater radius.
        // Only update up to 3 times the ejecta spread distance, for efficiency.
        // This is ok due to the fast dropoff in the ejecta.
        if (dist > 3 * crater.finalRadius)
          break;

        // The first NN will always point at the newly formed crater, so
        // ignore it:
        if (dist > 0) {
          double prevFloorElevation = cratersVector[it -> index]
                                      -> floorElevation;
          ...
          // Update crater depth
          cratersVector[it -> index] -> finalDepth -=
                                      (newFloorElevation - prevFloorElevation);

          // Update crater center elevation
          cratersVector[it -> index] -> floorElevation = newFloorElevation;
        }
    }
}
